...
@implementation MKMTFilterBeaconCellModel
@end

@interface MKMTFilterBeaconCell ()
...
@implementation MKMTFilterBeaconCell

+ (MKMTFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKMTFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKMTFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKMTFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKMTFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKMTFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKMTFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_mt_beaconMinValueChanged:index:)]) {
                [self.delegate mk_mt_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_mt_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_mt_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
