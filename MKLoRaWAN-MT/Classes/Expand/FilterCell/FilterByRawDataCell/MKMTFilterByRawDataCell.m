...
@implementation MKMTFilterByRawDataCellModel
...
@interface MKMTFilterByRawDataCell ()
...
@implementation MKMTFilterByRawDataCell

+ (MKMTFilterByRawDataCell *)initCellWithTableView:(UITableView *)tableView {
    MKMTFilterByRawDataCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKMTFilterByRawDataCellIdenty"];
    if (!cell) {
        cell = [[MKMTFilterByRawDataCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKMTFilterByRawDataCellIdenty"];
    }
    return cell;
}
...
- (void)textFieldValueChanged:(NSString *)text textType:(mk_mt_filterRawAdvDataTextType)type {
    if ([self.delegate respondsToSelector:@selector(mt_rawFilterDataChanged:index:textValue:)]) {
        [self.delegate mt_rawFilterDataChanged:type index:self.dataModel.index textValue:text];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKMTFilterByRawDataCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKMTFilterByRawDataCellModel.class]) {
        return;
    }
...
- (MKTextField *)typeTextField {
    if (!_typeTextField) {
        ...
        _typeTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            ...
            [sself textFieldValueChanged:text textType:mk_mt_filterRawAdvDataTextTypeDataType];
        };
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_mt_filterRawAdvDataTextTypeMinIndex];
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_mt_filterRawAdvDataTextTypeMaxIndex];
        };
    }
...
- (MKTextField *)rawDataField {
    if (!_rawDataField) {
        ...
        _rawDataField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_mt_filterRawAdvDataTextTypeRawDataType];
        };
    }
