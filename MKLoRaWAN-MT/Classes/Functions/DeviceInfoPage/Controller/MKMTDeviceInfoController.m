...
@interface MKMTDeviceInfoController ()<UITableViewDelegate,
UITableViewDataSource,
MKMTTextButtonCellDelegate>
...
@implementation MKMTDeviceInfoController

- (void)dealloc {
    NSLog(@"MKMTDeviceInfoController销毁");
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceStartDFUProcess)
                                                 name:@"mk_mt_startDfuProcessNotification"
                                               object:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKMTTextButtonCell *cell = [MKMTTextButtonCell initCellWithTableView:tableView];
...
#pragma mark - MKMTTextButtonCellDelegate
/// 用户点击了右侧按钮
/// @param index cell所在序列号
- (void)mt_textButtonCell_buttonAction:(NSInteger)index {
    if (index == 0) {
        //DFU
        MKMTUpdateController *vc = [[MKMTUpdateController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)pushSelftestInterface {
    MKMTSelftestController *vc = [[MKMTSelftestController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)updateCellDatas {
    ...
    if (ValidStr(self.dataModel.firmware)) {
        MKMTTextButtonCellModel *firmwareModel = self.section1List[0];
...
- (void)loadSection1Datas {
    MKMTTextButtonCellModel *cellModel = [[MKMTTextButtonCellModel alloc] init];
    cellModel.index = 0;
...
- (MKMTDeviceInfoModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMTDeviceInfoModel alloc] init];
    }
    return _dataModel;
}
