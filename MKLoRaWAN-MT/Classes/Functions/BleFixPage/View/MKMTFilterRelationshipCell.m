...
@implementation MKMTFilterRelationshipCellModel
@end

@interface MKMTFilterRelationshipCell ()
...
@implementation MKMTFilterRelationshipCell

+ (MKMTFilterRelationshipCell *)initCellWithTableView:(UITableView *)tableView {
    MKMTFilterRelationshipCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKMTFilterRelationshipCellIdenty"];
    if (!cell) {
        cell = [[MKMTFilterRelationshipCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@""];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataModel.dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(mt_filterRelationshipChanged:dataListIndex:)]) {
            [self.delegate mt_filterRelationshipChanged:self.dataModel.index dataListIndex:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKMTFilterRelationshipCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKMTFilterRelationshipCellModel.class]) {
        return;
    }
