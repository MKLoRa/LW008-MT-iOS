...
@interface MKMTVibrationController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMTVibrationDataModel *dataModel;

@end

@implementation MKMTVibrationController

- (void)dealloc {
    NSLog(@"MKMTVibrationController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MT", @"MKMTVibrationController", @"mt_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMTVibrationDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMTVibrationDataModel alloc] init];
    }
    return _dataModel;
}
