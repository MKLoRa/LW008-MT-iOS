//
//  MKMTFilterByAdvNameController.m
...
@interface MKMTFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMTFilterByAdvNameModel *dataModel;

@end

@implementation MKMTFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKMTFilterByAdvNameController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKMTFilterEditSectionHeaderViewModel *headerModel = [[MKMTFilterEditSectionHeaderViewModel alloc] init];
        headerModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MT", @"MKMTFilterByAdvNameController", @"mt_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMTFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMTFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
