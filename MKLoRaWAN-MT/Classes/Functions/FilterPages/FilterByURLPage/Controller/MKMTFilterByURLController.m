...
@interface MKMTFilterByURLController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMTFilterByURLModel *dataModel;

@end

@implementation MKMTFilterByURLController

- (void)dealloc {
    NSLog(@"MKMTFilterByURLController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MT", @"MKMTFilterByURLController", @"mt_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMTFilterByURLModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMTFilterByURLModel alloc] init];
    }
    return _dataModel;
}
