...
@interface MKMTFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMTFilterByMacModel *dataModel;

@end

@implementation MKMTFilterByMacController

- (void)dealloc {
    NSLog(@"MKMTFilterByMacController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MT", @"MKMTFilterByMacController", @"mt_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMTFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMTFilterByMacModel alloc] init];
    }
    return _dataModel;
}
