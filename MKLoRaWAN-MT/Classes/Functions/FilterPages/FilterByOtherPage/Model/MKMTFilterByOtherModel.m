...
@implementation MKMTFilterRawAdvDataModel
...
@interface MKMTFilterByOtherModel ()
...
@implementation MKMTFilterByOtherModel
...
- (BOOL)readFilterStatus {
    ...
    [MKMTInterface mt_readFilterByOtherStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKMTInterface mt_configFilterByOtherStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readRelationship {
    __block BOOL success = NO;
    [MKMTInterface mt_readFilterByOtherRelationshipWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configRelationship:(NSInteger)relationship {
    __block BOOL success = NO;
    [MKMTInterface mt_configFilterByOtherRelationship:relationship sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readConditions {
    __block BOOL success = NO;
    [MKMTInterface mt_readFilterByOtherConditionsWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configConditions:(NSArray <MKMTFilterRawAdvDataModel *>*)list {
    __block BOOL success = NO;
    [MKMTInterface mt_configFilterByOtherConditions:list sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
