...
@interface MKMTFilterByOtherController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKMTFilterEditSectionHeaderViewDelegate,
MKMTFilterByRawDataCellDelegate,
...
@property (nonatomic, strong)MKMTFilterByOtherModel *dataModel;

@end

@implementation MKMTFilterByOtherController

- (void)dealloc {
    NSLog(@"MKMTFilterByOtherController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKMTFilterEditSectionHeaderView *headerView = [MKMTFilterEditSectionHeaderView initHeaderViewWithTableView:tableView];
        MKMTFilterEditSectionHeaderViewModel *model = [[MKMTFilterEditSectionHeaderViewModel alloc] init];
        model.index = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKMTFilterByRawDataCell *cell = [MKMTFilterByRawDataCell initCellWithTableView:tableView];
...
#pragma mark - MKMTFilterEditSectionHeaderViewDelegate
/// 加号点击事件
/// @param index 所在index
- (void)mk_mt_filterEditSectionHeaderView_addButtonPressed:(NSInteger)index {
    ...
    MKMTFilterByRawDataCellModel *cellModel = [[MKMTFilterByRawDataCellModel alloc] init];
    if (cellModelIndex == 0) {
        //Condition A
...
#pragma mark - MKMTFilterByRawDataCellDelegate
/// 输入框内容发生改变
/// @param textType 哪个输入框发生改变了
/// @param index 当前cell所在的row
/// @param textValue 当前textField内容
- (void)mt_rawFilterDataChanged:(mk_mt_filterRawAdvDataTextType)textType
                          index:(NSInteger)index
                      textValue:(NSString *)textValue {
    if (index >= self.section1List.count) {
        return;
    }
    MKMTFilterByRawDataCellModel *cellModel = self.section1List[index];
    if (textType == mk_mt_filterRawAdvDataTextTypeDataType) {
        //过滤类型输入框内容发生改变
        cellModel.dataType = textValue;
        return;
    }
    if (textType == mk_mt_filterRawAdvDataTextTypeMinIndex) {
        //开始过滤的Byte索引输入框发生改变
        cellModel.minIndex = textValue;
        return;
    }
    if (textType == mk_mt_filterRawAdvDataTextTypeMaxIndex) {
        //截止过滤的Byte索引输入框发生改变
        cellModel.maxIndex = textValue;
        return;
    }
    if (textType == mk_mt_filterRawAdvDataTextTypeRawDataType) {
        //过滤内容输入框发生改变
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        MKMTFilterByRawDataCellModel *cellModel = self.section1List[i];
        MKMTFilterRawAdvDataModel *model = [[MKMTFilterRawAdvDataModel alloc] init];
...
- (mk_mt_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_mt_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_mt_filterByOther_AB;
        }
        return mk_mt_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_mt_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_mt_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_mt_filterByOther_AOrBOrC;
        }
    }
    return mk_mt_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MT", @"MKMTFilterByOtherController", @"mt_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMTFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMTFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
