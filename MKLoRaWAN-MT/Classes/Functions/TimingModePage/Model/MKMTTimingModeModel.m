...
@implementation MKMTTimingModeTimePointModel
@end

@interface MKMTTimingModeModel ()
...
@implementation MKMTTimingModeModel
...
- (BOOL)readStrategy {
    ...
    [MKMTInterface mt_readTimingModePositioningStrategyWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configStrategy {
    __block BOOL success = NO;
    [MKMTInterface mt_configTimingModePositioningStrategy:self.strategy sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readReportingTimePoint {
    __block BOOL success = NO;
    [MKMTInterface mt_readTimingModeReportingTimePointWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReportingTimePoint:(NSArray <MKMTTimingModeTimePointModel *>*)pointList {
    __block BOOL success = NO;
    [MKMTInterface mt_configTimingModeReportingTimePoint:pointList sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
