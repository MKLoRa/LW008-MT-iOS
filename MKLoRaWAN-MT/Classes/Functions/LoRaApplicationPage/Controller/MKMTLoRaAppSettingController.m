//
//  MKMTLoRaAppSettingController.m
...
@interface MKMTLoRaAppSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMTLoRaAppSettingModel *dataModel;

@end

@implementation MKMTLoRaAppSettingController

- (void)dealloc {
    NSLog(@"MKMTLoRaAppSettingController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MT", @"MKMTLoRaAppSettingController", @"mt_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMTLoRaAppSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMTLoRaAppSettingModel alloc] init];
    }
    return _dataModel;
}
