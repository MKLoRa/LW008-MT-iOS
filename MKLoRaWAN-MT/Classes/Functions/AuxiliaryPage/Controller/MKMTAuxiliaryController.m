...
@interface MKMTAuxiliaryController ()<UITableViewDelegate,
...
@implementation MKMTAuxiliaryController

- (void)dealloc {
    NSLog(@"MKMTAuxiliaryController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        MKMTDownlinkController *vc = [[MKMTDownlinkController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Vibration Detection
        MKMTVibrationController *vc = [[MKMTVibrationController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //ManDown Detection
        MKMTManDownController *vc = [[MKMTManDownController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Active State Count
        MKMTActiveStateController *vc = [[MKMTActiveStateController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
