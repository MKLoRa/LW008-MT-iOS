...
@implementation MKMTScanPageCell

+ (MKMTScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKMTScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKMTScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKMTScanPageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKMTScanPageCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(mt_scanCellConnectButtonPressed:)]) {
        [self.delegate mt_scanCellConnectButtonPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKMTScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKMTScanPageModel.class]) {
        return;
    }
    //顶部
...
- (UIImageView *)rssiIcon {
    if (!_rssiIcon) {
        ...
        _rssiIcon.image = LOADICON(@"MKLoRaWAN-MT", @"MKMTScanPageCell", @"mt_scan_rssiIcon.png");
    }
    return _rssiIcon;
}
...
- (UIImageView *)batteryIcon {
    if (!_batteryIcon) {
        ...
        _batteryIcon.image = LOADICON(@"MKLoRaWAN-MT", @"MKMTScanPageCell", @"mt_scan_batteryIcon.png");
    }
    return _batteryIcon;
}
