...
@interface MKMTActiveStateDataModel ()
...
- (BOOL)readActiveStateCount {
    ...
    [MKMTInterface mt_readActiveStateCountStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configActiveStateCount {
    __block BOOL success = NO;
    [MKMTInterface mt_configActiveStateCountStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readActiveStateTimeout {
    __block BOOL success = NO;
    [MKMTInterface mt_readActiveStateTimeoutWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configActiveStateTimeout {
    __block BOOL success = NO;
    [MKMTInterface mt_configActiveStateTimeout:[self.interval longLongValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
