...
@interface MKMTManDownDataModel ()
...
@implementation MKMTManDownDataModel
/*
...
- (void)resetIdleStatusWithSucBlock:(void (^)(void))sucBlock failedBlock:(void (^)(NSError *error))failedBlock {
    [MKMTInterface mt_configIdleStutasResetWithSucBlock:sucBlock failedBlock:failedBlock];
}

#pragma mark - interface
- (BOOL)readManDownDetection {
    __block BOOL success = NO;
    [MKMTInterface mt_readManDownDetectionWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configManDownDetection {
    __block BOOL success = NO;
    [MKMTInterface mt_configManDownDetectionStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readIdleDetectionTimeout {
    __block BOOL success = NO;
    [MKMTInterface mt_readIdleDetectionTimeoutWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configIdleDetectionTimeout {
    __block BOOL success = NO;
    [MKMTInterface mt_configIdleDetectionTimeout:[self.interval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
