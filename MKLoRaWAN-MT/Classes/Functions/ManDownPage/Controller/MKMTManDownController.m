...
@interface MKMTManDownController ()<UITableViewDelegate,
...
@implementation MKMTManDownController

- (void)dealloc {
    NSLog(@"MKMTManDownController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MT", @"MKMTManDownController", @"mt_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMTManDownDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMTManDownDataModel alloc] init];
    }
    return _dataModel;
}
