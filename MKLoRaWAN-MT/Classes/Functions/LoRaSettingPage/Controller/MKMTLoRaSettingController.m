...
@interface MKMTLoRaSettingController ()<UITableViewDelegate,
...
MKMTLoRaSettingAccountCellDelegate>
...
@property (nonatomic, strong)MKMTLoRaSettingModel *dataModel;

@end

@implementation MKMTLoRaSettingController

- (void)dealloc {
    NSLog(@"MKMTLoRaSettingController销毁");
}
...
- (void)rightButtonMethod {
    ...
    if (!ValidStr([MKMTUserLoginManager shared].password)) {
        //当前未登录
        @weakify(self);
        MKIoTCloudAccountLoginAlertViewModel *viewModel = [[MKIoTCloudAccountLoginAlertViewModel alloc] init];
        viewModel.account = [MKMTUserLoginManager shared].username;
        viewModel.isHome = [MKMTUserLoginManager shared].isHome;
        viewModel.password = [MKMTUserLoginManager shared].password;
    ...
    //登录过
    [self login:[MKMTUserLoginManager shared].isHome username:[MKMTUserLoginManager shared].username password:[MKMTUserLoginManager shared].password];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 7) {
        //Account
        return ((self.dataModel.configModel.supportServerPlatform && self.dataModel.platform == 1 && ValidStr([MKMTUserLoginManager shared].password)) ? self.section6List.count : 0);
    }
    if (section == 8) {
        //Gateway EUI(Gateway ID)
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2) {
        //mokoDevEUI
        MKMTDevEUICell *cell = [MKMTDevEUICell initCellWithTableView:tableView];
    ...
    if (indexPath.section == 7) {
        //Account
        MKMTLoRaSettingAccountCell *cell = [MKMTLoRaSettingAccountCell initCellWithTableView:tableView];
...
#pragma mark - MKMTLoRaSettingAccountCellDelegate
- (void)mt_loRaSettingAccountCell_logoutBtnPressed {
    MKIoTCloudExitAccountAlert *alert = [[MKIoTCloudExitAccountAlert alloc] init];
    BOOL isHome = [MKMTUserLoginManager shared].isHome;
    NSString *username = [MKMTUserLoginManager shared].username;
    [alert showWithAccount:username completeBlock:^{
        [[MKMTUserLoginManager shared] syncLoginDataWithHome:isHome username:username password:@""];
...
- (void)loadSection1Datas {
    MKMTDevEUICellModel *cellModel = [[MKMTDevEUICellModel alloc] init];
    NSString *macAddress = [[MKMTConnectModel shared].macAddress stringByReplacingOccurrencesOfString:@":" withString:@""];
...
- (void)loadSection6Datas {
    MKMTLoRaSettingAccountCellModel *cellModel = [[MKMTLoRaSettingAccountCellModel alloc] init];
    cellModel.account = [MKMTUserLoginManager shared].username;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MT", @"MKMTLoRaSettingController", @"mt_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMTLoRaSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMTLoRaSettingModel alloc] init];
    }
    return _dataModel;
}
