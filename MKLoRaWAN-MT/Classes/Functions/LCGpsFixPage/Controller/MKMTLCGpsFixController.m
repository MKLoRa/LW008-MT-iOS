...
@interface MKMTLCGpsFixController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMTLCGpsFixModel *dataModel;

@end

@implementation MKMTLCGpsFixController

- (void)dealloc {
    NSLog(@"MKMTLCGpsFixController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MT", @"MKMTLCGpsFixController", @"mt_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMTLCGpsFixModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMTLCGpsFixModel alloc] init];
    }
    return _dataModel;
}
