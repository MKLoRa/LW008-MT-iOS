...
@interface MKMTMotionModeController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMTMotionModeModel *dataModel;

@end

@implementation MKMTMotionModeController

- (void)dealloc {
    NSLog(@"MKMTMotionModeController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MT", @"MKMTMotionModeController", @"mt_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMTMotionModeModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMTMotionModeModel alloc] init];
    }
    return _dataModel;
}
