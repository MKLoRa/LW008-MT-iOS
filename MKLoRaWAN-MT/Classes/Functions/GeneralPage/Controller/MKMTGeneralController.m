...
@interface MKMTGeneralController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMTGeneralPageModel *dataModel;

@end

@implementation MKMTGeneralController

- (void)dealloc {
    NSLog(@"MKMTGeneralController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Device Mode
        MKMTDeviceModeController *vc = [[MKMTDeviceModeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 0) {
        //Auxiliary Operation
        MKMTAuxiliaryController *vc = [[MKMTAuxiliaryController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //Bluetooth Settings
        MKMTBleSettingsController *vc = [[MKMTBleSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 0) {
        //3-Axis Settings
        MKMTAxisSettingController *vc = [[MKMTAxisSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MT", @"MKMTGeneralController", @"mt_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMTGeneralPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMTGeneralPageModel alloc] init];
    }
    return _dataModel;
}
